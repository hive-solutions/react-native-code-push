package com.releasium.codepush.react;
...
public class ReleasiumCodePush implements ReactPackage {
    ...
    // Helper classes.
    private ReleasiumCodePushUpdateManager mUpdateManager;
    private ReleasiumCodePushTelemetryManager mTelemetryManager;
    ...
    private static ReleasiumCodePush mCurrentInstance;

    public ReleasiumCodePush(String deploymentKey, Context context) {
    ...
    public ReleasiumCodePush(String deploymentKey, Context context, boolean isDebugMode) {
        mContext = context.getApplicationContext();

        mUpdateManager = new ReleasiumCodePushUpdateManager(context.getFilesDir().getAbsolutePath());
        mTelemetryManager = new ReleasiumCodePushTelemetryManager(mContext);
        ...
        if (sAppVersion == null) {
            try {
                ...
                throw new ReleasiumCodePushUnknownException("Unable to get package info for " + mContext.getPackageName(), e);
            }
        }
    ...
    public ReleasiumCodePush(String deploymentKey, Context context, boolean isDebugMode, String serverUrl) {
        this(deploymentKey, context, isDebugMode);
        mServerUrl = serverUrl;
    }

    public ReleasiumCodePush(String deploymentKey, Context context, boolean isDebugMode, int publicKeyResourceDescriptor) {
        this(deploymentKey, context, isDebugMode);

        mPublicKey = getPublicKeyByResourceDescriptor(publicKeyResourceDescriptor);
    }

    public ReleasiumCodePush(String deploymentKey, Context context, boolean isDebugMode, String serverUrl, Integer publicKeyResourceDescriptor) {
        this(deploymentKey, context, isDebugMode);
    ...
    private String getPublicKeyByResourceDescriptor(int publicKeyResourceDescriptor){
        ...
        try {
            ...
            throw new ReleasiumCodePushInvalidPublicKeyException(
        ...
        if (publicKey.isEmpty()) {
            throw new ReleasiumCodePushInvalidPublicKeyException("Specified public key is empty");
        }
        return publicKey;
    }
    ...
    private String getCustomPropertyFromStringsIfExist(String propertyName) {
        ...
        int resId = mContext.getResources().getIdentifier("ReleasiumCodePush" + propertyName, "string", packageName);
        ...
        if (resId != 0) {
            ...
            if (!property.isEmpty()) {
                ...
                ReleasiumCodePushUtils.log("Specified " + propertyName + " is empty");
            } 
        }
    ...
    long getBinaryResourcesModifiedTime() {
        try {
            ...
            throw new ReleasiumCodePushUnknownException("Error in getting binary resources modified time", e);
        }
    }
    ...
    public static String getJSBundleFile() {
        return ReleasiumCodePush.getJSBundleFile(CodePushConstants.DEFAULT_JS_BUNDLE_NAME);
    }

    public static String getJSBundleFile(String assetsBundleFileName) {
        if (mCurrentInstance == null) {
            throw new ReleasiumCodePushNotInitializedException("A ReleasiumCodePush instance has not been created yet. Have you added it to your app's list of ReactPackages?");
        }
    ...
    public String getJSBundleFileInternal(String assetsBundleFileName) {
        ...
        } catch (ReleasiumCodePushMalformedDataException e) {
            // We need to recover the app in case 'codepush.json' is corrupted
            ReleasiumCodePushUtils.log(e.getMessage());
            clearUpdates();
        }

        if (packageFilePath == null) {
            // There has not been any downloaded updates.
            ReleasiumCodePushUtils.logBundleUrl(binaryJsBundleUrl);
        ...
        if (isPackageBundleLatest(packageMetadata)) {
            ReleasiumCodePushUtils.logBundleUrl(packageFilePath);
    ...
    void initializeUpdateAfterRestart() {
        ...
        if (pendingUpdate != null) {
            ...
            try {
                ...
                // We need to recover the app in case 'codepush.json' is corrupted
                ReleasiumCodePushUtils.log(e);
                clearUpdates();
                return;
            }
            if (packageMetadata == null || !isPackageBundleLatest(packageMetadata) && hasBinaryVersionChanged(packageMetadata)) {
                ReleasiumCodePushUtils.log("Skipping initializeUpdateAfterRestart(), binary version is newer");
                return;
            }
            ...
            try {
                ...
                if (updateIsLoading) {
                    ...
                    ReleasiumCodePushUtils.log("Update did not finish loading the last time, rolling back to a previous version.");
                ...
                // Should not happen.
                throw new ReleasiumCodePushUnknownException("Unable to read pending update metadata stored in SharedPreferences", e);
            }
        }
    }
    ...
    private boolean isPackageBundleLatest(JSONObject packageMetadata) {
        try {
            ...
            throw new ReleasiumCodePushUnknownException("Error in reading binary modified date from package metadata", e);
        }
    }
    ...
    public void setNeedToReportRollback(boolean needToReportRollback) {
        ReleasiumCodePush.sNeedToReportRollback = needToReportRollback;
    }
