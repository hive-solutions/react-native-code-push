...
export type SyncStatusChangedCallback = (status: ReleasiumCodePush.SyncStatus) => void;
export type HandleBinaryVersionMismatchCallback = (update: RemotePackage) => void;

export interface ReleasiumCodePushOptions extends SyncOptions {
    /**
     * Specifies when you would like to synchronize updates with the ReleasiumCodePush server.
     * Defaults to releasiumCodePush.CheckFrequency.ON_APP_START.
     */
    checkFrequency: ReleasiumCodePush.CheckFrequency;
}
...
export interface LocalPackage extends Package {
     ...
     */
    install(installMode: ReleasiumCodePush.InstallMode, minimumBackgroundDuration?: number): Promise<void>;
}
...
export interface Package {
    ...
    /**
     * The internal label automatically given to the update by the ReleasiumCodePush server. This value uniquely identifies the update within its deployment.
     */
    label: string;
...
export interface RemotePackage extends Package {
    /**
     * Downloads the available update from the ReleasiumCodePush service.
     *
...
export interface SyncOptions {
     ...
     * Defaults to releasiumCodePush.InstallMode.ON_NEXT_RESTART.
     */
    installMode?: ReleasiumCodePush.InstallMode;

    /**
     * Specifies when you would like to install updates which are marked as mandatory.
     * Defaults to releasiumCodePush.InstallMode.IMMEDIATE.
     */
    mandatoryInstallMode?: ReleasiumCodePush.InstallMode;
...
export interface StatusReport {
     ...
     */
    status: ReleasiumCodePush.DeploymentStatus;
...
/**
 * Decorates a React Component configuring it to sync for updates with the ReleasiumCodePush server.
 *
 ...
 */
declare function ReleasiumCodePush(options?: ReleasiumCodePushOptions): (x: any) => any;
 ...
 */
declare function ReleasiumCodePush(x: any): any

declare namespace ReleasiumCodePush {
    /**
    ...
    /**
     * Asks the ReleasiumCodePush service whether the configured app deployment has an update available.
     *
     * @param deploymentKey The deployment key to use to query the ReleasiumCodePush server for an update.
     * 
    ...
    /**
     * Notifies the ReleasiumCodePush runtime that an installed update is considered successful.
     */
    function notifyAppReady(): Promise<StatusReport|void>;

    /**
     * Allow ReleasiumCodePush to restart the app.
     */
    function allowRestart(): void;

    /**
     * Forbid ReleasiumCodePush to restart the app.
     */
    function disallowRestart(): void;

    /**
     * Clear all downloaded ReleasiumCodePush updates.
     * This is useful when switching to a different deployment which may have an older release than the current package.
     * Note: we don't recommend to use this method in scenarios other than that (ReleasiumCodePush will call
    ...
    enum SyncStatus {
        /**
         * The app is up-to-date with the ReleasiumCodePush server.
         */
        UP_TO_DATE,
        ...
        /**
         * The ReleasiumCodePush server is being queried for an update.
         */
        CHECKING_FOR_UPDATE,
        ...
        /**
         * An available update is being downloaded from the ReleasiumCodePush server.
         */
        DOWNLOADING_PACKAGE,
    ...
    /**
     * Indicates when you would like to check for (and install) updates from the ReleasiumCodePush server.
     */
    enum CheckFrequency {
        /**
        ...
        /**
         * Don't automatically check for updates, but only do it when releasiumCodePush.sync() is manully called inside app code.
         */
        MANUAL
    }
}

export default ReleasiumCodePush;
